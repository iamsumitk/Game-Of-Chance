//	Note no one is signuped at first you have to signup first.
...
}user[2],temp;	//user[2] for 2 users especially in EBird.
...
void add(int *s)
{
	unsigned int acash; // adding cash or money
	printf("\nEnter the amount to add in your account");
	fflush(stdin);
	scanf("%d",&acash);
	*s = *s + acash;	
	printf("\nAmmount updated new balance %d\n",*s);
	getch();
	return;
}
...
void Initial(void){
	...
	c = getch();
	if (c == '1'){
		SignUp(0);	// 0 for the 2 users playing 0 for 1st and 1 for 2nd
	}
	else if (c == '2')
	    Login(0);
	else if ( c == 27){
...
void SignUp(int s){			// s is a parameter for 1st or second user.
    system("cls");
	int i,j;
    FILE*fp;
    ...
    user[s].PEBird[0] = 0;	user[s].PSlots[0] = 0;	user[s].PRoulette[0] = 0;			//initial probability of all users in
    user[s].PEBird[1] = 0;	user[s].PSlots[1] = 0;	user[s].PRoulette[1] = 0;			//all games is 0
...
void Login (int s){
	...
	if(num1 == 0 && num2 == 0){
		...
		break;
			...
			printf("Sorry you do not have sufficiant balance to play the games");
			printf("\nIf You want to add money in account press enter or press any other key to exit");
			int ch = getch();
			if ( ch==13 ){
			add(&user[s].amount);	
			Login(s);	}
...
void Menu(void){
	...
	printf("\nDo You Want To Play More in this casino...?\n"
...
void EBird(void){
	...
	int ch, x;
	unsigned int bamount;	//bamount - betting amount.
	printf("\nEBird game requires two players\n"
	...
	if(ch == '1') Login(1);
   	...
   	StartE:
   	system("cls");	
	printf("\nEnter a betting amount for the game (betting amount will be same for both players).\n");
   	scanf("%u",&bamount); fflush(stdin);
   	if(bamount<=user[0].amount)  
 {
 		if (bamount<=user[1].amount){
    ...
    user[0].amount-=bamount;	user[1].amount-=bamount;		//betting amount is detected from their accounts.
    key:
    ...
    if (c == 224){
        ...
        if(c1 == 72 || c1 == 80 || c1 == 75 || c1 == 77){
		    if(u2==c1){
				...
				user[1].amount=user[1].amount+(2*bamount);		//increment in winners account
   		 	    printf("/tWinning amount = %d (including your detected amount)",2*bamount);
				user[1].PEBird[0]+=1;	
		    }
        }
    }
	if (c == 'w' || c == 'a' || c == 's' || c == 'd' ){
        if(u1 == c){
    	    ...
    	    user[0].amount=user[0].amount+(2*bamount);		//increment in winners account
    	    printf("/tWinning amount = %d (including your detected amount)",2*bamount);
			user[0].PEBird[0]+=1;
	}}
	...
	if(ch == 13)	EBird();
	      ...
	      printf("\nInsufficiant amount! Mr. %s for this game\n",user[1].name);	
		  printf("if you want to add money press enter or press any other key to exit");
	      int ch = getch();
		  if( ch == 13)
	      add(&user[1].amount);
		  else	{
...
void SlotColor(int x){
    ...
    int i, x[3] , j , count = 0 ;char n;    
     unsigned int sbamount;            //sbamount=sloting betting amount
	printf("\t****************Slots & Jackpots**************** "); printf("\n");
	printf("\nEnter a betting amount for the game.\n");
   	fflush(stdin);		scanf("%u",&sbamount); 
	FILE *fp,*fs;
   	StartS:
   	if(sbamount<user[0].amount)  
 {
    fp = fopen("data.dat","ab+");
    fs = fopen("temp.dat","ab+");	
    
    printf("\t\nPress Enter to start the game\n");
	n = getch(); user[0].amount-=sbamount;		//betting amount is detected from their accounts.
    if(n == 13){
        srand(time(0));
        for(i=0;i<=10;i++){
	    ...
	    for(j=0;j<3;j++){SlotColor(x[j]);printf("\t$%d$\t",x[j]);SetColor(15);}
	        ...
	        if (x[0] == x[1] && x[0] == x[2]){
		        ...
		        count = 0;
              user[0].amount=user[0].amount+(3*sbamount); 
              printf("/tWinning amount is three times greater = %d (including your detected amount)",3*sbamount);
		        user[0].PSlots[0]+=1;
	        }
	        else printf("\n\n\tSorry! Hard Luck!\n\n\n"); 
	        printf( "\n\tProbability of Winning = %.2f%", //next line as well ...
			ProbCal(user[0].PSlots[0],user[0].PSlots[1]) );
			printf("\nYou have %d amount left in your account",user[0].amount); 
            struct data update;			//to update the data. 
            while ( fread(&update , sizeof(update) , 1 , fp) ){
            	if(strcmp(user[0].username , update.username)==0 ){
            		fwrite(&user[0],sizeof(user[0]),1,fs);
